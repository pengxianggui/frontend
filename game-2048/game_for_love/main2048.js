...
$(document).ready(function() {
	var touchblock = document.querySelector("#touchArea");
	newgame();//HTML文档加载时，只是去开始一个新的函数：newgame()

	touchblock.addEventListener('touchstart', function(event){touchstart(event)}, false);

	touchblock.addEventListener('touchend', function(event){touchend(event)}, false);
...
function touchstart(event){
	if (event.targetTouches.length === 1) {//单指触碰时
		// alert("touchstart");
		var touch = event.targetTouches[0];
		pressPoint[0] = touch.pageX;
		pressPoint[1] = touch.pageY;
	}
}
...
function touchend(event) {
	// if (event.targetTouches.length === 1) {
		...
		switch(moveDirection(event, pressPoint)) {
			case 37: //玩家按下左箭头
			console.log("左移：37");
...
function isSuccess() {
	...
	if (boardMax(board) >= 2048) {
		...
		alert("恭喜小妮子成功通关！想找上帝的密码吗？关闭本弹框，在2048游戏的棋盘上 鼠标右击 -> 查看网页源代码");
	}
}
