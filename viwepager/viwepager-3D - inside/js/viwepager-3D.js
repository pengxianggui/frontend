window.onload = function() {
	...
	// controlDiv[0].addEventListener("mouseover", function() {rotateByHover(0, controlDiv)});
	// controlDiv[1].addEventListener("mouseover", function() {rotateByHover(1, controlDiv)});
	// controlDiv[2].addEventListener("mouseover", function() {rotateByHover(2, controlDiv)});
	// controlDiv[3].addEventListener("mouseover", function() {rotateByHover(3, controlDiv)});
	
	//设置图片的自动播放
	setInterval(rotateAuto, 6000);

	//下面是监听鼠标是否移动到id为parent的div块上，需要的逻辑是，一旦鼠标移动上去了，就不会自动切换
	block.addEventListener("mouseover", function() {ishover = true});
	block.addEventListener("mouseout", function() {ishover = false});
